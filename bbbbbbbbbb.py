...
class CONTIG:
    ...
    def get_most(self):
        ...
        for _rname, _blockN in self.blockN_DICT.items():
            if _blockN > blockN:
                blockN = _blockN
                rname = _rname
    ...
    def calc(self, rname, coverage):
        ...
        print("KKKKKKKKK", len(interceptP_LIST), len(interceptM_LIST))
...
for contig in engine.run():
    ...
    qname = contig.qname
    rname = contig.get_most()

    rsize = rFAR.get(rname)
    qsize = qFAR.get(qname)

    #print(contig.qname, rname, contig.blockN_DICT[rname])
    ...
    qname, strand, intercept, number, variance, std_deviation, qsPos, qePos, rsPos, rePos, coverage = contig.calc(rname, 1)
