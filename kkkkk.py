...
class CONTIG:
    ...
    def add(self, block):
        ...
        if not block.intercept in self.block_DICT[block.strand]:
            self.block_DICT[block.strand][block.intercept]  = [block]
        else:
            self.block_DICT[block.strand][block.intercept] += [block]
    
    def remove(self, minBlockN):
        _contig = CONTIG(self.qname, self.rname)
        for strand, sub2_DICT in self.block_DICT.items():
        ...
        self.block_LIST = _contig.block_LIST

    def calc(self, coverage):
        target_LIST = []
        for strand, sub1_DICT in self.block_DICT.items():
        ...
        for qname, group1 in groupby(self.fin, lambda line: line.split('_')[0]):
            ...
            for key2, group2 in groupby(group1, lambda line: line.split('\t')[0]):
                ...
                for data in group2:
                    ...
                    temp = (rname, flag, qpos, rpos)

                    if not rname in temp_DICT: temp_DICT[rname] = []
                    temp_DICT[rname] += [temp]
            
            tempN = 0
            rname = 0
            temp_LIST = None
            for _rname, _temp_LIST in temp_DICT.items():
                if len(_temp_LIST) > tempN :
                    tempN = len(_temp_LIST)
                    rname = _rname
                    temp_LIST = _temp_LIST
            
            contig = CONTIG(qname, rname)
            for temp in temp_LIST:
                rname, flag, qpos, rpos = temp
                rpos = int(rpos)
...
for contig in engine.run():
    contig.remove(1000/READ_SLIDING)
    ...
    qname, strand, intercept, number, variance, std_deviation, qsPos, qePos, rsPos, rePos, coverage = contig.calc(1)
    ...
    for strand, sub_DICT in contig.block_DICT.items():
...
for rname, image in image_DICT.items():
    fout = open('dotplot4/{0}.html'.format(rname), 'w')
