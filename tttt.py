...
class CONTIG:
    ...
    def filter(self, minBlockN):
        for rname, sub1_DICT in self.block_DICT.items():
            for strand, sub2_DICT in sub1_DICT.items():
                keys_to_remove = [intercept for intercept, block_LIST in sub2_DICT.items() if minBlockN > len(block_LIST)]
                #print(rname, keys_to_remove[0:5])
                for key in keys_to_remove:
                    del sub2_DICT[key]

    def filter2(self, minBlockN):
    ...
    def calc(self, rname, coverage):
        ...
        if len(interceptP_LIST) > len(interceptM_LIST):
            STRAND = '+'
            meanIntercept = sum(interceptP_LIST) / len(interceptP_LIST)
            variance = sum((x - meanIntercept) ** 2 for x in interceptP_LIST) / len(interceptP_LIST)
            std_deviation = variance ** 0.5
        else:
            STRAND = '-'
            meanIntercept = sum(interceptM_LIST) / len(interceptM_LIST)
            variance = sum((x - meanIntercept) ** 2 for x in interceptM_LIST) / len(interceptM_LIST)
            std_deviation = variance ** 0.5
        ...
        return self.qname, rname, STRAND, meanIntercept, number, variance, std_deviation, qsPos, qePos, rsPos, rePos, float(qePos - qsPos) / (rePos - rsPos)
...
class IMAGE:
    def __init__(self, seqName, seqLen):
        ...
        self.group.attr('transform', 'translate({0},{1}) '.format(margin, margin))
        self.set_ruler()
...
class DOTPLOT:
    ...
    def add_block(self, block):
        ...
        dot.attr('cx', "{0:.2f}".format(cx))
        dot.attr('cy', "{0:.2f}".format(cy))
        dot.attr('r', r)
...
for contig in engine.run():
    filteredContig = contig.filter2(5)
    qname = filteredContig.qname
    qSize = qFAR.get(qname)
    qSumSize += qSize

    #percentage
    jobTimer.check()
    percentage = float(qSumSize)/qFAR.totalN
    print("Read File... [{0:6.2f}%] remainTime: {1}".format(percentage*100, jobTimer.remainTime(percentage)))

    rname_LIST = filteredContig.block_DICT.keys()
    for rname in rname_LIST:
        _qname, _rname, strand, intercept, number, variance, std_deviation, qsPos, qePos, rsPos, rePos, coverage = filteredContig.calc(rname,0.95)
        
        #print(qname, rname, "{0:>20.5f}    {1:>20.5f}    {2:>20}     {3:>20.5f}     {4:>20.5f}       {5:}".format(std_deviation, coverage, number, float(number*71)/(qePos - qsPos), float(number*71)/(qSize), chi_squared_test(variance, number)))
        if chi_squared_test(variance, number) == False: continue
        if 0.01 > float(number*READ_SLIDING)/(qSize): continue

        if not rname in image_DICT:
            image = IMAGE(rname, rFAR.get(rname))
            image_DICT[rname] = image
        else:
            image = image_DICT[rname]

        if not qname in image.dotplot_DICT:
            image.add(qname, qFAR.get(qname))

        dotplot = image.get(qname)

        dotplot.set_position(strand, intercept)
        dotplot.set_border(1, qFAR.get(qname), rsPos, rePos)

        for strand, sub_DICT in filteredContig.block_DICT[rname].items():
